...
class TimeMachineCollectionViewLayout: UICollectionViewLayout {
    ...
    /// The height of the cells. Callers provide a value that matches their
    /// content instead of coupling the shared layout to a concrete cell type.
    ...
    required init?(coder: NSCoder) {
        let decodedHeight = coder.decodeDouble(forKey: "itemHeight")
        self.itemHeight = decodedHeight > 0 ? decodedHeight : 1
        super.init(coder: coder)
    }

    override func encode(with coder: NSCoder) {
        super.encode(with: coder)
        coder.encode(Double(self.itemHeight), forKey: "itemHeight")
    }
    ...
    override func prepare() {
        ...
        self.cellLayoutAttributes.removeAll(keepingCapacity: true)
        let currentPosition = Double(self.continuousFocusedPosition)
    ...
    /// The continuously interpolated item position represented by the current
    /// scroll offset. Consumers such as onboarding progress indicators use
    /// this value without reimplementing the Time Machine geometry.
    var continuousFocusedPosition: CGFloat {
        guard self.itemHeight > 0 else { return 0 }
        return max(0, min(self.zPosition / self.itemHeight, CGFloat(max(0, self.numberOfItems(inSection: 0) - 1))))
    }

    /// Returns the currently frontmost visible cell without imposing a
    /// concrete message-cell type on the shared layout.
    func getFrontmostCell<Cell: UICollectionViewCell>(as cellType: Cell.Type) -> Cell? {
        guard let indexPath = self.getFrontmostIndexPath() else { return nil }
